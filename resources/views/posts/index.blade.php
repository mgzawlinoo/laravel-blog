            ...
            <div class="col-md-12">
                ...
                @isset($q)
                    <div class="alert alert-info d-flex justify-content-between">
                        Search Result for : {{ $q }} <a href="{{ route('posts.index') }}" class="btn btn-secondary btn-sm">Clear</a>
                    </div>
                @endisset
                        ...
                        <tbody>
                            @foreach ($posts as $post)
                                <tr>
                                    ...
                                    <td class="text-start">{{ $post->category->name }}</td>
                                    <td class="text-start">{{ $post->user->name }}</td>
                                    <td class="text-start">{{ $post->updated_at->diffForHumans() }}</td>
                                    <td class="text-end">
                                        <a href="{{ route('posts.show', $post->slug) }}" class="d-inline btn btn-primary btn-sm"><i class="text-white fa-regular fa-eye"></i></a>
                                        <a href="{{ route('posts.edit', $post->slug) }}" class="d-inline btn btn-warning btn-sm"><i class="text-white fa-regular fa-pen-to-square"></i></a>
